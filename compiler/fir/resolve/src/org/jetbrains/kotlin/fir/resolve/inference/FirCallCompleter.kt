...
class FirCallCompleter(
    ...
    private inner class LambdaAnalyzerImpl : LambdaAnalyzer {
        override fun analyzeAndGetLambdaReturnArguments(
            ...
            val matchedParameter = candidate.argumentMapping?.firstNotNullOfOrNull { (argument, valueParameter) ->
                val matchedArg =
                    ((argument as? FirLambdaArgumentExpression)?.expression as? FirAnonymousFunctionExpression)?.anonymousFunction?.takeIf { it === lambdaArgument }
                if (matchedArg == null) null
                else valueParameter
            }

            lambdaArgument.originalAnonymousFunctionType = matchedParameter?.returnTypeRef?.coneType
